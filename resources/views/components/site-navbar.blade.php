...
<nav x-data="{ open: false }" {{ $attributes->merge(['class' => $navClass]) }}>
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div class="hidden flex-1 justify-center px-6 md:flex">
            <div class="relative w-full max-w-md" data-live-search data-search-url="{{ route('search') }}" data-suggest-url="{{ route('search.suggest') }}">
                ...
                <div data-search-dropdown class="absolute right-0 z-50 mt-2 hidden w-[min(92vw,640px)] overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-800 shadow-2xl">
                    <ul data-search-results class="max-h-[32rem] overflow-y-auto"></ul>
                    <a data-search-see-more href="{{ route('search') }}" class="block border-t border-slate-200 px-4 py-3 text-xs font-bold uppercase tracking-wide text-violet-700 hover:bg-slate-50">
                        See more results
                    </a>
                </div>
            </div>
    ...
    <div x-cloak x-show="open" x-transition class="mx-4 mt-4 rounded-2xl border border-white/10 bg-[#1c1527]/95 p-4 shadow-2xl backdrop-blur md:hidden">
        <div class="relative" data-live-search data-search-url="{{ route('search') }}" data-suggest-url="{{ route('search.suggest') }}">
            ...
            <div data-search-dropdown class="absolute left-0 right-0 z-50 mt-2 hidden overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-800 shadow-2xl">
